	...
	// The rate at which the menu expands revealing child elements on clickpri
	$.menu_speed = 235;
 ...
 * loadScript("assets/js/my_lovely_script.js", myPrettyCode);
 */
...
function gotoPage(pageToGo, totalPage){

    if(pageToGo < 1 || pageToGo > totalPage){
        return false;
    }

    $('#CurrentPage').val(pageToGo);
    
    //Click button
    $('#SearchByCriteriaBTN').trigger('click');

    return false;
}

//Force user to stay on current page after refresh page.
function SetActivePage(){

    $('#CurrentPage').val($('#AvtivePage').val());
...
function validFileExtension(inputObj, extensions){
    ...
    if ( file ) {
        ...
        // check file type is valid as given in 'extensions' array
        if ( $.inArray ( get_ext[0].toLowerCase(), extensions ) > -1 ){
            return true;
        }
    }
    return false;
}
function validFileSize(inputObj, maxSizeKB){
    var kbSize = ($(inputObj)[0].files[0].size / 1024);
    if(kbSize > maxSizeKB)
        return false;
    else
        return true;
}
function getFileSizeMB(inputObj){
    return (Math.ceil($(inputObj)[0].files[0].size / 1024 / 1024)) + ' MB';
}
function getFileSizeKB(inputObj){
    return (Math.ceil($(inputObj)[0].files[0].size / 1024)) + ' kB';
}
function getFileName(inputObj){
    var file = $(inputObj).val();
    // first check if file field has any value
    if ( file ) {
        // split file name at dot
        ...
        //get extension.
        var ext = get_ext[0].toLowerCase();
        get_ext = get_ext[1].split('\\');
        get_ext = get_ext.reverse();

        return  get_ext[0] +'.'+ ext;
    }
    return "";
}
...
//sadsada.tam create 2014-12-15
function RequestResultGetMenu(res) {
    if (res.success) {
        var grps = res.grps;
        var grpsHTML = '';

        var query_grps_root = Enumerable.From(grps)
              .Where('$.Link_GroupId == "" ')
              .OrderBy('$.GroupId')
              .ToArray();

        for (i = 0; i < query_grps_root.length; i++) {
            var grp = query_grps_root[i];

            var grpId = grp.GroupId;
            var grpName = grp.Name;
            var grpPath = grp.Path == '' ? '#' : grp.Path; 
            var grpIcon = grp.Icon;

            var subGrp = getGrpHTML(grps, grpId); 
            var iconGrp = grpIcon != '' ? grpIcon : 'fa-tasks'; //default icon : fa-tasks.

            grpsHTML += ' <li>  ' +
                            ' <a href="' + grpPath + '"><i class="fa fa-lg fa-fw ' + iconGrp + '"></i> <span class="menu-item-parent">' + grpName + '</span> </a>' +
                            '' + subGrp + '' +
                            ' </li> ';
        }

        if (grpsHTML != "") {
            grpsHTML = ' <ul>' + grpsHTML + '</ul> ';

            //Add hind left menu button.
            grpsHTML += '<span class="minifyme" data-action="minifyMenu" style=""> <i class="fa fa-arrow-circle-left hit"></i> </span>';
        }

        $('#MainNavigator').html(grpsHTML);
        ...
        //error
        alert('Render Menu Error : ' + res.message);
    }
}

//sadsada.tam create 2014-12-15
function getGrpHTML(grps, rootGrpCode) {
    var grpsHTML = '';

    var query_grps = Enumerable.From(grps)
                  .Where('$.Link_GroupId == "' + rootGrpCode + '"')
                  .OrderBy('$.GroupId')
                  .ToArray();

    if (query_grps.length == 0) {
        return "";
    }

    for (var j = 0; j < query_grps.length; j++) {
        var grp = query_grps[j];

        var grpCode = grp.GroupId;
        var grpLink = grp.Link_GroupId;
        var grpName = grp.Name;
        var grpPath = grp.Path == '' ? '#' : grp.Path; 
        var grpIcon = grp.Icon;

        var subGrp = getGrpHTML(grps, grpCode); //get recursive.
        var iconGrp = grpIcon != '' ? grpIcon : 'fa-tasks'; //default icon : fa-tasks.
...
function setConterntToUrl() {
            
}

 function myprint() {
     var a = window.open('', '', 'scrollbars=yes,width=' + screen.width + ' ,height='+screen.height);
	a.document.open("text/html");
	a.document.write("<html><head>");
	a.document.write('<link rel="stylesheet" type="text/css"  href="Assets/css/bootstrap.min.css">');
	a.document.write('<link rel="stylesheet" type="text/css"  href="Assets/css/font-awesome.min.css">');
	a.document.write('<link rel="stylesheet" type="text/css"  href="Assets/css/smartadmin-production.css">');
	a.document.write('<link rel="stylesheet" type="text/css"  href="Assets/css/smartadmin-skins.css">');
	a.document.write('<link rel="stylesheet" type="text/css"  href="Assets/css/ustyle.css">');
	a.document.write('<body>');
	a.document.write("<table width='100%'><tr><td width='50%' overflow:auto><div style='width:100%; max-height:"+screen.height+"; overflow:auto'>");
 
	a.document.write('<link rel="stylesheet" type="text/css"  media="screen" href="Assets/css/print.css">');
	a.document.write('<link rel="stylesheet" type="text/css"  media="print" href="Assets/css/print.css">');
	a.document.write('<a href="#" onclick="print()" class="btn btn-default hidden-print pull-right">print</a>');	
	a.document.write($('#content').html());
	a.document.write("</td><td width='50%'>");
	a.document.write("XXX");
	a.document.write("</td></tr></table>");
	a.document.write('<div id="test"></div>');
	a.document.write('<script src="Assets/js/libs/jquery-2.0.2.min.js" type="text/javascript" charset="utf-8"></script>');
	a.document.write('<script src="Assets/js/myprint.js" type="text/javascript" charset="utf-8"></script>');
	a.document.write('<script type="text/javascript" charset="utf-8">$(document).ready(init());</script>');
